...
function initNiceSidebar() {
	...
	li.click(function() {
		...
		if (e.hasClass('active')) {
			...
			if (!initState) {
			...
			else { // list was just initialized
				...
				li.each((index, item) => {
					...
					if (item.attr('class') != e.attr('class')) {
						
						// deactivate tag
						item.removeClass('active')
						item.css('color', grey)
						tags.filter((ti, te) => te.innerHTML === item.html()).css('background-color', grey)
					}
				})
			}
		}
